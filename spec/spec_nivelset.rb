...
RSpec.describe ServicioSanitario::NivelSet do
    ...
    it "Se espera que se devuleva la alerta en función del tiempo " do
        expect(ServicioSanitario.obtener_nivel(5)).to eq(ServicioSanitario::AZUL)
        expect(ServicioSanitario.obtener_nivel(15)).to eq(ServicioSanitario::ROJO)
        expect(ServicioSanitario.obtener_nivel(40)).to eq(ServicioSanitario::NARANJA)
        expect(ServicioSanitario.obtener_nivel(50)).to eq(ServicioSanitario::VERDE)
        expect(ServicioSanitario.obtener_nivel(70)).to eq(ServicioSanitario::NEGRO)
    end
